...
2. For on-chain mining, run **`check_wallet.py`** before bootstrapping or trials. Pass **`--project-id`** or **`--token-address`**. If `ARAH_PRIVATE_KEY` is missing, ask the user to put a `.env` file in the current working directory with `ARAH_PRIVATE_KEY=0x...` and optional `ARAH_STAKE=1` (whole tokens; ProjectToken `decimals() == 0`), then rerun preflight. Stop if the wallet cannot sign, has no native gas, or cannot cover/buy the missing ProjectToken stake. When `ARAH_STAKE` is absent, use the default `1` — the contract only requires `stake > 0`.
