...
2. For Solana projects, read `references/onchain-mining-solana.md`. Use `download_irys_artifacts.mjs` when given hashes or `storage_irys.json`. Do not run 0G wallet preflight or 0G proposal submit scripts for Solana projects; use `submit_trial_proposal.py --chain solana` after an improvement.
