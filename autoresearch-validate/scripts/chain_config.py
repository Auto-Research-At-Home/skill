...
VALIDATE_ROOT = Path(__file__).resolve().parent.parent
DEFAULT_DEPLOYMENT = VALIDATE_ROOT / "contracts" / "0g-galileo-testnet" / "deployment.json"
...
def deployment_dir(deployment_path: Path) -> Path:
    return deployment_path.parent.resolve()
...
def verifier_registry_address(deployment: dict[str, Any]) -> str:
    return os.environ.get("ARAH_VERIFIER_REGISTRY") or deployment["contracts"]["VerifierRegistry"]["address"]
